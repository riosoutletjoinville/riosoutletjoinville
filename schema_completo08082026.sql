...
\restrict BNE5agjJoIyegWhPeXEJGJbnEhlGb6mcdysR8OyJ9D6gspAAuUuPRTHy3arKdhp
...
--
-- Name: atualizar_status_parcela(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.atualizar_status_parcela() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    total_pago numeric;
...
BEGIN
    SELECT valor_parcela INTO valor_parcela_atual
    ...
    UPDATE pre_pedido_parcelas 
    SET 
        saldo_restante = GREATEST(valor_parcela_atual - total_pago, 0),
        status = CASE 
            WHEN valor_parcela_atual - total_pago <= 0 THEN 'pago'
...
--
-- Name: cancelar_pedido_completo(uuid, uuid, text, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.cancelar_pedido_completo(p_pedido_id uuid, p_usuario_id uuid, p_motivo text, p_tipo_cancelamento text DEFAULT 'arrependimento'::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  v_total NUMERIC;
  v_status TEXT;
  v_pre_pedido_id UUID;
  item_record RECORD;
BEGIN
  -- 1. Buscar dados do pedido
  SELECT total, status, pre_pedido_id INTO v_total, v_status, v_pre_pedido_id
  FROM pedidos
  WHERE id = p_pedido_id;
  ...
  IF v_status = 'cancelado' THEN
    RETURN jsonb_build_object('error', 'Pedido já está cancelado');
  END IF;
  
  -- 2. Remover financeiro existente (se houver)
  DELETE FROM financeiro WHERE pedido_id = p_pedido_id;
  
  -- 3. Restaurar estoque (se for pedido que baixou estoque)
  IF v_status IN ('confirmado', 'pendente', 'processando', 'troca_solicitada') THEN
    FOR item_record IN 
      SELECT pi.produto_id, pi.variacao_id, pi.quantidade, pi.preco_unitario
      FROM pedido_itens pi
      WHERE pi.pedido_id = p_pedido_id
    LOOP
      UPDATE produtos 
      SET estoque = estoque + item_record.quantidade,
          updated_at = NOW()
      WHERE id = item_record.produto_id;
      
      INSERT INTO baixas_estoque (
        produto_id, variacao_id, quantidade, motivo,
        preco_unitario, data_baixa, tipo_ajuste,
        observacao, pedido_id, usuario_id
      ) VALUES (
        item_record.produto_id,
        item_record.variacao_id,
        item_record.quantidade,
        'arrependimento',
        item_record.preco_unitario,
        NOW(),
        'entrada',
        'Estorno por cancelamento - Pedido ' || p_pedido_id,
        p_pedido_id,
        p_usuario_id
      );
    END LOOP;
  ...
  -- 4. Atualizar o pedido (ignorando erro do trigger)
  BEGIN
    UPDATE pedidos 
    SET 
      status = 'cancelado',
      tipo_cancelamento = p_tipo_cancelamento,
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Se o trigger falhar, tentar novamente com uma abordagem diferente
    -- Registrar o erro, mas continuar
    RAISE NOTICE 'Erro ao atualizar pedido: %', SQLERRM;
  END;
  
  -- 5. Verificar se o pedido foi cancelado
  IF v_status != 'cancelado' THEN
    -- Tentar atualizar novamente sem triggers (usando SQL puro)
    -- Nota: isso pode não funcionar se o trigger for ROW LEVEL
    PERFORM set_config('session.replication_role', 'replica', true);
    ...
    PERFORM set_config('session.replication_role', 'origin', true);
  END IF;
  
  -- 6. Inserir estorno no financeiro (tentar novamente)
  BEGIN
    INSERT INTO financeiro (
      tipo, descricao, valor, categoria, data_movimento,
      pedido_id, tipo_movimento, status, created_at
    ) VALUES (
      'estorno',
      'Estorno - Cancelamento Pedido #' || substr(p_pedido_id::text, 1, 8),
      -ABS(v_total),
      'estornos',
      CURRENT_DATE,
      p_pedido_id,
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Erro ao inserir estorno no financeiro: %', SQLERRM;
  END;
  
  -- 7. Resultado
  RETURN jsonb_build_object(
    'success', true,
    'message', 'Pedido cancelado com sucesso (com avisos)',
...
--
-- Name: notificar_mudanca_estoque(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.notificar_mudanca_estoque() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    ...
    v_titulo TEXT;
BEGIN
    -- Só executa se o estoque realmente mudou
    IF NEW.estoque IS DISTINCT FROM OLD.estoque THEN
        
        -- Determinar nível anterior (baseado no OLD.estoque)
        IF OLD.estoque <= 0 THEN
            v_nivel_anterior := 3;
        ELSIF OLD.estoque <= 5 THEN
            v_nivel_anterior := 2;
        ELSIF OLD.estoque <= 10 THEN
            v_nivel_anterior := 1;
        ELSE
            v_nivel_anterior := 0;
        END IF;
        
        -- Determinar nível novo (baseado no NEW.estoque)
        IF NEW.estoque <= 0 THEN
            v_nivel_novo := 3;
            v_titulo := 'Produto sem Estoque';
            v_mensagem := NEW.titulo || ' - Estoque ESGOTADO!';
            v_deve_notificar := v_nivel_novo > v_nivel_anterior;
            
        ELSIF NEW.estoque <= 5 THEN
            v_nivel_novo := 2;
            v_titulo := 'Estoque Crítico';
            v_mensagem := NEW.titulo || ' - APENAS ' || NEW.estoque || ' unidades!';
            -- Notifica ao entrar no nível 2, e também se veio do nível 0 (reabastecimento crítico)
            v_deve_notificar := v_nivel_novo > v_nivel_anterior OR (v_nivel_anterior = 3 AND v_nivel_novo = 2);
            
        ELSIF NEW.estoque <= 10 THEN
            v_nivel_novo := 1;
            v_titulo := 'Estoque Baixo';
            v_mensagem := NEW.titulo || ' - Apenas ' || NEW.estoque || ' unidades restantes';
            -- Notifica apenas quando entra no nível 1 (se não estava já em nível crítico)
            v_deve_notificar := v_nivel_novo > v_nivel_anterior AND v_nivel_anterior < 1;
            
        ELSE
            v_nivel_novo := 0;
            -- Se estoque normalizou, limpar última notificação
            IF OLD.ultimo_nivel_notificado > 0 THEN
                NEW.ultimo_nivel_notificado := 0;
                NEW.ultima_notificacao_estoque := NULL;
            END IF;
        END IF;
        ...
        IF v_deve_notificar THEN
            INSERT INTO public.notifications (
                tipo, titulo, mensagem, metadata, created_at
            ) VALUES (
                'estoque',
                v_titulo,
                v_mensagem,
                jsonb_build_object(
                    ...
                    'nivel_notificado', v_nivel_novo
                ),
                NOW()
            );
            
            -- Atualizar controle da última notificação
            NEW.ultimo_nivel_notificado := v_nivel_novo;
            NEW.ultima_notificacao_estoque := NOW();
        END IF;
    END IF;
...
CREATE FUNCTION public.pedido_item_estoque_trigger() RETURNS trigger
    ...
    AS $$
DECLARE
    v_nao_baixar_estoque boolean;
BEGIN
    -- Buscar a flag nao_baixar_estoque do pré-pedido associado
    SELECT COALESCE(pp.nao_baixar_estoque, false) INTO v_nao_baixar_estoque
    FROM pedidos p
    LEFT JOIN pre_pedidos pp ON p.pre_pedido_id = pp.id
    WHERE p.id = NEW.pedido_id;
    
    -- Se nao_baixar_estoque for TRUE, NÃO baixar o estoque
    IF v_nao_baixar_estoque = true THEN
        RAISE NOTICE '⚡ Estoque NÃO será baixado para o pedido % (flag nao_baixar_estoque = true)', NEW.pedido_id;
        RETURN NEW;
    END IF;
    
    -- Se não tiver flag ou for false, baixar normalmente
    IF NEW.variacao_id IS NOT NULL THEN
        UPDATE produto_variacoes 
        SET estoque = estoque - NEW.quantidade
        WHERE id = NEW.variacao_id;
        
        RAISE NOTICE '✅ Estoque baixado da variação %: % unidades', NEW.variacao_id, NEW.quantidade;
    ELSE
        UPDATE produtos 
        SET estoque = estoque - NEW.quantidade
        WHERE id = NEW.produto_id;
        
        RAISE NOTICE '✅ Estoque baixado do produto %: % unidades', NEW.produto_id, NEW.quantidade;
    END IF;
...
--
-- Name: registrar_movimento_financeiro_correto(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.registrar_movimento_financeiro_correto() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  -- Agora aceita 'confirmado', 'pago' e 'approved'
  IF (NEW.status IN ('confirmado', 'pago', 'approved')) 
     AND (OLD.status IS DISTINCT FROM NEW.status) 
  THEN
    
    -- Verificar se já existe registro financeiro para este pedido
    IF NOT EXISTS (SELECT 1 FROM financeiro WHERE pedido_id = NEW.id) THEN
      
      -- Inserir movimento de ENTRADA (positivo)
      INSERT INTO financeiro (
        tipo,
        descricao,
        valor,
        ...
        'recebimento',
        'Venda - Pedido #' || substr(NEW.id::text, 1, 8),
        NEW.total,
        'vendas',
        COALESCE(NEW.data_pedido, CURRENT_DATE)::date,
        NEW.id,
        'entrada',  
        NEW.status,
        NEW.payment_id,
...
CREATE TABLE public.notas_fiscais (
    ...
    ref_focus character varying(100)
);
...
--
-- Name: parcela_negociacoes; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.parcela_negociacoes (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    parcela_id uuid NOT NULL,
    valor_original numeric(10,2) NOT NULL,
    valor_negociado numeric(10,2) NOT NULL,
    desconto_concedido numeric(10,2) NOT NULL,
    novo_vencimento date,
    observacao text,
    usuario_id uuid,
    data_negociacao timestamp with time zone DEFAULT now(),
    status character varying(20) DEFAULT 'ativo'::character varying
);


--
-- Name: parcela_pagamentos; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.parcela_pagamentos (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    parcela_id uuid NOT NULL,
    ...
    usuario_id uuid,
    created_at timestamp with time zone DEFAULT now(),
    tipo_movimento character varying(20) DEFAULT 'parcial'::character varying
);
...
CREATE TABLE public.pre_pedido_parcelas (
    ...
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    valor_original numeric(10,2) DEFAULT NULL::numeric,
    saldo_restante numeric(10,2) DEFAULT NULL::numeric,
    status_parcela character varying(30) DEFAULT 'pendente'::character varying,
    negociado boolean DEFAULT false,
    observacao_negociacao text,
    CONSTRAINT pre_pedido_parcelas_status_check CHECK (((status)::text = ANY ((ARRAY['pendente'::character varying, 'pago'::character varying, 'atrasado'::character varying, 'cancelado'::character varying, 'parcial'::character varying])::text[])))
);
...
CREATE TABLE public.pre_pedidos (
    ...
    nao_baixar_estoque boolean DEFAULT false,
...
CREATE TABLE public.produtos (
    ...
    ultima_notificacao_estoque timestamp with time zone,
    ultimo_nivel_notificado integer DEFAULT 0
);


--
-- Name: COLUMN produtos.ultima_notificacao_estoque; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.produtos.ultima_notificacao_estoque IS 'Última data que uma notificação de estoque foi enviada';


--
-- Name: COLUMN produtos.ultimo_nivel_notificado; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.produtos.ultimo_nivel_notificado IS '0=nunca, 1=abaixo_10, 2=abaixo_5, 3=zerado';
...
--
-- Name: parcela_negociacoes parcela_negociacoes_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.parcela_negociacoes
    ADD CONSTRAINT parcela_negociacoes_pkey PRIMARY KEY (id);


--
-- Name: parcela_pagamentos parcela_pagamentos_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.parcela_pagamentos
    ADD CONSTRAINT parcela_pagamentos_pkey PRIMARY KEY (id);
...
--
-- Name: financeiro unique_pedido_financeiro; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.financeiro
    ADD CONSTRAINT unique_pedido_financeiro UNIQUE (pedido_id);
...
--
-- Name: idx_notas_fiscais_pedido_id; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_notas_fiscais_pedido_id ON public.notas_fiscais USING btree (pedido_id);
...
--
-- Name: idx_parcela_negociacoes_parcela_id; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_parcela_negociacoes_parcela_id ON public.parcela_negociacoes USING btree (parcela_id);


--
-- Name: idx_parcela_pagamentos_parcela_id; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_parcela_pagamentos_parcela_id ON public.parcela_pagamentos USING btree (parcela_id);
...
--
-- Name: idx_pedidos_created_at_desc; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_pedidos_created_at_desc ON public.pedidos USING btree (created_at DESC);
...
--
-- Name: idx_pre_pedidos_nao_baixar_estoque; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_pre_pedidos_nao_baixar_estoque ON public.pre_pedidos USING btree (nao_baixar_estoque);
...
--
-- Name: parcela_pagamentos trigger_atualizar_status_parcela; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trigger_atualizar_status_parcela AFTER INSERT ON public.parcela_pagamentos FOR EACH ROW EXECUTE FUNCTION public.atualizar_status_parcela();
...
CREATE TRIGGER trigger_movimento_financeiro_pedido AFTER UPDATE ON public.pedidos FOR EACH ROW EXECUTE FUNCTION public.registrar_movimento_financeiro_correto();
...
CREATE TRIGGER trigger_notify_estoque_baixo AFTER UPDATE OF estoque ON public.produtos FOR EACH ROW EXECUTE FUNCTION public.notificar_mudanca_estoque();
...
--
-- Name: parcela_negociacoes parcela_negociacoes_parcela_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.parcela_negociacoes
    ADD CONSTRAINT parcela_negociacoes_parcela_id_fkey FOREIGN KEY (parcela_id) REFERENCES public.pre_pedido_parcelas(id) ON DELETE CASCADE;


--
-- Name: parcela_negociacoes parcela_negociacoes_usuario_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.parcela_negociacoes
    ADD CONSTRAINT parcela_negociacoes_usuario_id_fkey FOREIGN KEY (usuario_id) REFERENCES auth.users(id);


--
-- Name: parcela_pagamentos parcela_pagamentos_parcela_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.parcela_pagamentos
    ADD CONSTRAINT parcela_pagamentos_parcela_id_fkey FOREIGN KEY (parcela_id) REFERENCES public.pre_pedido_parcelas(id) ON DELETE CASCADE;


--
-- Name: parcela_pagamentos parcela_pagamentos_usuario_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.parcela_pagamentos
    ADD CONSTRAINT parcela_pagamentos_usuario_id_fkey FOREIGN KEY (usuario_id) REFERENCES auth.users(id);
...
\unrestrict BNE5agjJoIyegWhPeXEJGJbnEhlGb6mcdysR8OyJ9D6gspAAuUuPRTHy3arKdhp
