...
function getDataEmissaoBrasilia(): string {
  ...
  return dataBrasilia.toISOString().split('T')[0]; // YYYY-MM-DD
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const dadosFocus: any = {
      ...
      data_emissao: getDataEmissaoBrasilia(),
      tipo_documento: "1",
      ...
      logradouro_destinatario: (cliente.endereco || "NAO INFORMADO").substring(
        0,
        60,
      ),
      ...
      items: pedido.itens.map((item: any, idx: number) => {
        ...
        const codigoProduto = (
          item.produto?.modelo_prod ||
          item.produto?.modelo ||
          item.produto?.id ||
          "0001"
        ).substring(0, 60);
        const descricaoProduto = (item.produto?.titulo || "Produto").substring(
          0,
          120,
        );
    ...
    // Verificar se é rejeição direta pelo cStat
    const cStatStr = respostaFocus.status_sefaz
      ? String(respostaFocus.status_sefaz)
      : "";
    const isRejeicaoDireta =
      cStatStr.startsWith("2") ||
      cStatStr.startsWith("3") ||
      cStatStr.startsWith("4");

    if (respostaFocus.status === "autorizado" || cStatStr === "100") {
      // AUTORIZADO
      dadosCompletos = {
        ...dadosCompletos,
        ...
        mensagem_sefaz: respostaFocus.mensagem_sefaz || respostaFocus.xMotivo,
    ...
    } else if (isRejeicaoDireta || respostaFocus.status === "rejeitado") {
      // REJEITADO DIRETO (cStat 237, 703, etc)
      console.log(
        "❌ Nota rejeitada diretamente:",
        respostaFocus.mensagem_sefaz || respostaFocus.xMotivo,
      );
      dadosCompletos = {
        ...dadosCompletos,
        chave: respostaFocus.chave_nfe,
        ...
        mensagem_sefaz:
          ...
          respostaFocus.xMotivo ||
          respostaFocus.motivo,
        caminho_xml: respostaFocus.caminho_xml_nota_fiscal,
        caminho_danfe: respostaFocus.caminho_danfe,
      };
      statusFinal = "rejeitado";
    } else if (respostaFocus.status === "processando_autorizacao") {
      // PROCESSANDO - entra no loop de consulta
      ...
      while (!consultado && tentativas < maxTentativas) {
        ...
        try {
          ...
          const cStatConsulta = consulta.status_sefaz
            ? String(consulta.status_sefaz)
            : "";
          const isRejeicaoConsulta =
            cStatConsulta.startsWith("2") ||
            cStatConsulta.startsWith("3") ||
            cStatConsulta.startsWith("4");

          if (consulta.status === "autorizado" || cStatConsulta === "100") {
            dadosCompletos = {
              ...dadosCompletos,
              chave: consulta.chave_nfe,
              protocolo: consulta.protocolo || consulta.nProt,
              status_sefaz: consulta.status_sefaz,
              mensagem_sefaz: consulta.mensagem_sefaz || consulta.xMotivo,
            ...
            break;
          } else if (isRejeicaoConsulta || consulta.status === "rejeitado") {
            dadosCompletos = {
              ...dadosCompletos,
              chave: consulta.chave_nfe,
              protocolo: consulta.protocolo || consulta.nProt,
              status_sefaz: consulta.status_sefaz,
              mensagem_sefaz:
                consulta.mensagem_sefaz || consulta.xMotivo || consulta.motivo,
              caminho_xml: consulta.caminho_xml_nota_fiscal,
              caminho_danfe: consulta.caminho_danfe,
            };
            statusFinal = "rejeitado";
            ...
            console.log(
              "❌ Nota rejeitada:",
              consulta.mensagem_sefaz || consulta.xMotivo,
            );
            break;
            ...
            consultado = true;
            break;
          } else if (consulta.status === "denegado") {
            dadosCompletos = {
              ...dadosCompletos,
              chave: consulta.chave_nfe,
              protocolo: consulta.protocolo || consulta.nProt,
...
function mapearStatusParaBanco(status: string, statusSefaz?: string): string {
  // Primeiro, verificar se é uma rejeição (cStat começando com 2, 3 ou 4)
  if (statusSefaz) {
    const cStatStr = String(statusSefaz);

    // Rejeições: 2xx, 3xx, 4xx
    if (
      cStatStr.startsWith("2") ||
      cStatStr.startsWith("3") ||
      cStatStr.startsWith("4")
  ...
  if (status === "autorizado" || statusSefaz === "100") {
    ...
    status === "rejeitado" ||
    statusSefaz?.startsWith("2") ||
    statusSefaz?.startsWith("3") ||
    statusSefaz?.startsWith("4")
  ) {
    return "rejeitada";
