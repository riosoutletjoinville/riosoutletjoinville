...
export async function POST(request: NextRequest) {
  try {
    ...
    // Calcular soma dos produtos (sem frete)
    ...
    if (isMesmoEstado) {
      ...
      if (isCPF) {
        cfop = "5102";
        icmsSituacaoTributaria = "102"; // Tributada sem benefício fiscal (consumidor final)
      } else if (isCNPJ) {
        cfop = "5102";
        icmsSituacaoTributaria = "101"; // Tributada com benefício fiscal
      }
    } else {
      // Fora do estado (interestadual)
      if (isCPF) {
        cfop = "6108"; // Venda a consumidor final interestadual
        icmsSituacaoTributaria = "102"; // Tributada sem benefício fiscal
      } else if (isCNPJ) {
        cfop = "6101"; // Venda a contribuinte interestadual
        icmsSituacaoTributaria = "101"; // Tributada com benefício fiscal
      }
    }

    // Fallback caso não tenha identificado
    if (!cfop) {
    ...
    // Agora use as variáveis no seu dadosFocus:
    const dadosFocus = {
      ...
      valor_produtos: somaProdutos.toFixed(2),
      ...
      items: pedido.itens.map((item: any, idx: number) => {
        ...
        const freteRateado = (valorItem / somaProdutos) * valorFrete;
        return {
          numero_item: (idx + 1).toString(),
          codigo_produto:
            item.produto?.modelo_prod || item.produto?.modelo || "0123456789-x",
          ...
          valor_frete: freteRateado.toFixed(2),
