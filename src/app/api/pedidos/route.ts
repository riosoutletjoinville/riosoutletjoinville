...
export async function POST(request: Request) {
  ...
  console.log("⏰ Horário:", getBrasiliaISOString());
  console.log("========================================");
  ...
  try {
    ...
    const pedidoData = {
      ...
      status: "confirmado",
      data_pedido: getBrasiliaISOString(),
...
for (const item of prePedido.itens as ItemPedido[]) {
  ...
  const { data: produtoExiste, error: produtoCheckError } = await supabase
    .from("produtos")
    .select("id")
    .eq("id", item.produto.id)
    .maybeSingle();

  if (produtoCheckError || !produtoExiste) {
  ...
  for (const [chaveTamanho, quantidade] of Object.entries(item.tamanhos)) {
    ...
    const { tamanho: nomeTamanho, cor: nomeCor } = extrairTamanhoECor(chaveTamanho);
...
// Se houver produtos não encontrados, avisar mas continuar
if (produtosNaoEncontrados.length > 0) {
  console.warn("⚠️ Produtos não encontrados:", produtosNaoEncontrados);
  // Opcional: retornar aviso ao usuário
}
...
if (itensInseridos.length > 0) {
  ...
  // Se não houver itens para inserir, deletar o pedido criado
  console.warn("⚠️ Nenhum item válido para inserir. Deletando pedido.");
  await supabase.from("pedidos").delete().eq("id", pedido.id);
  return NextResponse.json(
    { error: "Nenhum produto válido encontrado para criar o pedido" },
    { status: 400 },
  );
}
