...
export default function CarrinhoContent() {
  ...
  useEffect(() => {
    if (carrinho.length > 0 && (frete?.opcao_selecionada || frete?.opcoes_disponiveis?.length)) {
      handleLimparFrete();
    }
  ...
  const handleFinalizarCompra = async (e: React.MouseEvent) => {
    ...
    if (!podeFinalizar()) {
      ...
      if (carrinho.length === 0) {
        ...
        mensagem = "Por favor, digite seu CEP e calcule o frete antes de finalizar a compra.";
        titulo = "Calcular Frete";
      } else if (Object.values(estoqueInsuficiente).some((v) => v)) {
        mensagem = "Alguns itens no carrinho não possuem estoque suficiente. Por favor, ajuste as quantidades.";
  ...
  const handleQuantidadeChange = async (
    ...
    const sucesso = await atualizarQuantidade(produtoId, variacaoId, novaQuantidade);
