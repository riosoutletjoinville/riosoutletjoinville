...
function CheckoutContent() {
  ...
  useEffect(() => {
    const pedidoSalvo = sessionStorage.getItem("pedido_nao_logado_atual");
    if (pedidoSalvo) {
  ...
  const handleTipoCadastro = async (
    ...
    if (pedidoCriado) {
      setEtapa("pagamento");
      setMetodoPagamento(metodo || null);
      return;
    }
    ...
    try {
      ...
      if (resultado.success) {
        ...
        setEtapa("pagamento");
        setMetodoPagamento(metodo || null);
        if (metodo === "pix" && resultado.pedido_id) {
          setPixReady(true);
        }
      } else {
          ...
          <div className="max-w-2xl mx-auto">
            <Card>
              <CardContent className="pt-6">
                ...
                {/* Conteúdo direto, sem escolha de método */}
                {metodoPagamento === "cartao" ? (
                  <>
                    {bricksError && (
