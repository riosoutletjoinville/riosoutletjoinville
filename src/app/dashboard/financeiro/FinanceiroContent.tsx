...
export default function FinancePage() {
  ...
  const loadMovimentos = useCallback(async () => {
    try {
      ...
      if (filtroTipo !== "todos") {
        countQuery = countQuery.eq("tipo_movimento", filtroTipo);
      }
      ...
      if (filtroTipo !== "todos") {
        query = query.eq("tipo_movimento", filtroTipo);
      }
  ...
  const movimentosEntrada = movimentos.filter((m) => m.tipo_movimento === "entrada");
  const movimentosSaida = movimentos.filter(
    (m) => m.tipo_movimento === "saida" && !m.categoria.toLowerCase().includes("estorno"),
  );
