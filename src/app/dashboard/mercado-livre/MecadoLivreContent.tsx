// src/app/dashboard/mercado-livre/MercadoLivreContent.tsx
"use client";
...
export default function MercadoLivreContent() {
  ...
  // ✅ Crie o cliente browser (que tem acesso às cookies)
  const supabase = createClient();

  const loadConnection = async () => {
    try {
      setLoading(true);
      console.log('Carregando conexão...');
      
      // ✅ Primeiro, verificar se o usuário está autenticado
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) {
        console.log("Nenhuma sessão encontrada");
        setError("Você precisa estar logado para conectar com o Mercado Livre.");
        setLoading(false);
        return;
      }
      ...
      // ✅ Passe o cliente browser para o serviço
  ...
  useEffect(() => {
    loadConnection();
  }, []); // Remova router das dependências

  // Efeito para detectar sucesso na URL
  useEffect(() => {
    const mlSuccess = searchParams.get("ml_success");
    if (mlSuccess === "connected") {
      console.log("Conexão bem-sucedida detectada! Recarregando...");
      loadConnection();
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      {error && (
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
          {error}
          <button
            onClick={() => router.push('/dashboard/mercado-livre/auth')}
      ...
      {isConnected && connectionInfo ? (
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <Link
            href="/dashboard/mercado-livre/auth"
