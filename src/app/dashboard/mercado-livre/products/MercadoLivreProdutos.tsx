// src/app/dashboard/mercado-livre/products/MercadoLivreProdutos.tsx
"use client";
...
export default function MercadoLivreProdutos() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // PASSO 2: Verificar conexão com ML (PASSANDO O CLIENT)
        const isConnected = await mercadoLivreService.isUserConnected(supabase);

        if (!isConnected) {
          setError(
            "Não conectado ao Mercado Livre. Conecte sua conta primeiro.",
          );
          setLoading(false);
          return;
        }

        // ✅ PASSO 3: Buscar produtos (PASSANDO O CLIENT)
        const productsData = await mercadoLivreService.getProducts(supabase);
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (!session) {
        setError("❌ Você não está logado no sistema. Faça login primeiro.");
        setLoading(false);
        return;
      }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link
              href="/dashboard/mercado-livre/products/sync"
