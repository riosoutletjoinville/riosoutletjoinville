...
export default function NFeDetalhePage() {
  ...
  // Substitua a função handleConsultar
  async function handleConsultar() {
    setConsultando(true);
    const loadingAlert = notification.showLoading("Consultando SEFAZ...");

    try {
      // Usar a nova rota específica de consulta
      const response = await fetch(`/api/nfe/${nfe?.id}/consultar`);
      ...
      if (result.success) {
        await carregarNFe(); // Recarregar dados atualizados

        notification.showInfo(
          "Consulta realizada",
          result.data
            ? `Status: ${result.status_atual}`
            : "Nenhuma alteração no status",
        );
      }
    } catch (error) {
      await Swal.close();
      ...
      notification.showError(
        ...
        error instanceof Error
          ? error.message
          : "Erro ao consultar situação da NF-e",
      );
    } finally {
  ...
  async function handleCancelar() {
    ...
    setCancelando(true);
    notification.showLoading("Cancelando NF-e...");

    try {
      // NOVA ROTA CORRETA
      const response = await fetch(
        `/api/nfe/${nfe?.id}/cancelar?justificativa=${encodeURIComponent(justificativa)}`,
        { method: "DELETE" },
      );
