...
import { formatBrasiliaDate, formatBrasiliaTime, getBrasiliaISOString } from "@/lib/dateUtils";
...
import { NFeStatusBadge } from "@/components/dashboard/nfe/NFeStatusBadge";
import AcoesPedidoModal from "@/components/dashboard/pedidos/AcoesPedidoModal";
import { PedidosTableSkeleton } from "@/components/dashboard/pedidos/PedidosTableSkeleton";
...
export default function PedidosContent() {
  ...
  try {
    // Buscar todos os pré-pedidos de uma vez com os dados relacionados
    const { data, error } = await supabase
      .from("pre_pedidos")
      .select(
        `
    ...
    // Buscar TODOS os usuários necessários de uma só vez
    const userIds = (data || [])
    ...
    // Processar os dados com os usuários já carregados
    const prePedidosComUsuarios = (data || []).map((item) => {
      const usuarioData = item.usuario_id ? usuariosMap.get(item.usuario_id) : null;
  ...
  const handleSavePrePedido = async (pedidoData: PedidoSaveData) => {
    try {
      ...
      const prePedidoData = {
        ...
        nao_baixar_estoque: pedidoData.naoBaixarEstoque || false,
        created_at: getBrasiliaISOString(),
  ...
  try {
    ...
    const response = await fetch("/api/pedidos", {
      ...
      body: JSON.stringify({
        ...
        tipoPedidoId,
        justificativaTipo,
        naoBaixarEstoque: prePedido.nao_baixar_estoque || false,
      }),
    });
    ...
    if (!response.ok) {
      // Verificar se é erro de produtos inválidos
      if (data.produtosInvalidos && data.produtosInvalidos.length > 0) {
        let mensagem = "⚠️ Produtos inválidos encontrados:\n\n";
        data.produtosInvalidos.forEach((p: any, index: number) => {
          mensagem += `${index + 1}. ${p.titulo}\n   Motivo: ${p.motivo}\n\n`;
        });
        mensagem += "\nDeseja excluir este pré-pedido?";
        ...
        const result = await Swal.fire({
          ...
          confirmButtonText: "Sim, excluir pré-pedido",
          cancelButtonText: "Manter",
        });

        if (result.isConfirmed) {
          await handleExcluirPrePedidoInvalido(prePedido.id);
        }
        return;
      }

      if (data.error === "Estoque insuficiente" && data.produto) {
        Swal.fire(
          "Estoque insuficiente",
          `Produto ${data.produto} possui apenas ${data.disponivel} unidades em estoque (solicitado: ${data.solicitado})`,
          "error",
        );
      } else {
    ...
    Swal.fire(
      "Sucesso!",
      data.message || "Pedido confirmado com sucesso!",
      "success",
    );
    ...
    // Fechar modal e mudar para aba de pedidos
  ...
  const handleDownloadPedidoPDF = async (pedido: Pedido) => {
    try {
      ...
      if (!data.pedido_itens || data.pedido_itens.length === 0) {
        ...
        Swal.fire(
          "Erro",
          "Este pedido não possui itens para gerar o PDF",
          "error",
        );
        return;
      }
      ...
      const pdfData = {
        ...
        localTrabalho:
          data.local_trabalho_ped || data.cliente?.local_trabalho || "",
  ...
  const handleCancelPrePedido = async (prePedido: PrePedido) => {
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          status: "cancelado",
          cancelado_em: getBrasiliaISOString(),
      ...
      for (const ajuste of itensParaAjustar) {
        ...
        const { error: baixaError } = await supabase
          ...
          .insert({
            ...
            preco_unitario: item.preco_unitario,
            data_baixa: getBrasiliaISOString(),
            usuario_id: user?.id,
  ...
  const handleExcluirPrePedidoInvalido = async (prePedidoId: string) => {
  try {
    const result = await Swal.fire({
      title: "Excluir Pré-Pedido?",
      text: "Este pré-pedido contém produtos que não existem mais ou estão desativados. Deseja excluí-lo?",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#d33",
      cancelButtonColor: "#3085d6",
    ...
    Swal.fire("Excluído!", "Pré-pedido excluído com sucesso.", "success");
    loadPrePedidos();
    setSelectedPrePedido(null);
  } catch (error) {
    console.error("Erro ao excluir pré-pedido:", error);
    Swal.fire("Erro", "Não foi possível excluir o pré-pedido", "error");
  }
};
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {currentPedidos.map((pedido) => (
                        <tr key={pedido.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {formatBrasiliaTime(pedido.created_at)}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {formatBrasiliaDate(pedido.data_pedido )}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
