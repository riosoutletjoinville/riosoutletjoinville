...
export function NFeList({ nfes, onRefresh }: NFeListProps) {
  ...
  `/api/nfe/${nfeSelecionada.id}/cancelar?justificativa=${encodeURIComponent(justificativa)}`,
  { method: "DELETE" }
);
...
{(nfe.status === "autorizada" || nfe.status === "processando") && (
  <Button
    variant="ghost"
    size="icon"
    onClick={() => handleCancelarClick(nfe)}
    className="text-destructive hover:text-destructive"
    title={nfe.status === "processando" ? "Cancelar envio e liberar pedido" : "Cancelar NF-e"}
  >
