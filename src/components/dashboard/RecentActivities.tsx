  ...
  const loadRecentActivities = async () => {
    try {
      ...
      const { data: recentOrders } = await supabase
        .from("pedidos")
        .select("id, total, data_pedido, status, cliente_id, clientes!left(nome, razao_social, tipo_cliente)")
      ...
      // Adicionar pedidos recentes - MELHORADO: inclui nome do cliente
      recentOrders?.forEach((order) => {
        // Buscar nome do cliente
        let clienteNome = "Cliente";
        if (order.clientes) {
          const cliente = order.clientes as any;
          if (cliente.tipo_cliente === 'fisica') {
            clienteNome = `${cliente.nome} ${cliente.razao_social || ''}`.trim() || "Cliente";
          } else {
        ...
        const statusEmoji = order.status === 'pendente' ? '⏳' : order.status === 'pago' ? '✅' : '📦';
        
        activitiesList.push({
          id: order.id,
          type: "pedido",
          description: `#${order.id.slice(-6)} - ${clienteNome} - ${statusEmoji} R$ ${valorFormatado}`,
      ...
      lowStockProducts?.forEach((product) => {
        activitiesList.push({
          id: product.id,
          type: "estoque",
          description: `⚠️ ${product.titulo} (${product.estoque} unidades)`,
          time: "Agora",
      ...
      recentFinancial?.forEach((movement) => {
        ...
        const valorFormatado = movement.valor.toFixed(2).replace('.', ',');
        activitiesList.push({
          id: movement.id,
          type: "financeiro",
          description: `${tipo}: ${movement.descricao} - R$ ${valorFormatado}`,
      ...
      setActivities([
        ...
        },
        {
          id: "2",
          type: "pedido",
          description: "🛒 #123456 - João Silva - R$ 250,00",
          time: "5 min atrás",
          icon: <ShoppingCart className="h-4 w-4" />,
          color: "text-blue-600"
        },
