...
export default function SelecionarCondicaoPagamentoModal({
  ...
  const [condicaoSelecionadaId, setCondicaoSelecionadaId] =
    useState<string>("");
  ...
  const calcularParcelas = (condicaoId: string) => {
    ...
    // Função para criar data no fuso horário local (sem offset UTC)
    const criarDataLocal = (dataStr?: string): Date => {
      if (usarDataPersonalizada && dataStr) {
        // Parse da data no formato YYYY-MM-DD para evitar problemas de timezone
        const [year, month, day] = dataStr.split("-").map(Number);
        // Criar data no timezone local (month é 0-index)
        return new Date(year, month - 1, day);
      }
      // Data atual no timezone local
      const hoje = new Date();
      return new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate());
    };

    const dataBase = criarDataLocal(dataPrimeiraParcela);

    for (let i = 0; i < condicao.numero_parcelas; i++) {
      const dataVencimento = new Date(dataBase);

      if (i === 0) {
        // Primeira parcela usa a data base (já está no timezone local)
        // Não precisa fazer nada
      } else {
        // Para as demais parcelas, avança meses mantendo o dia do mês
        dataVencimento.setMonth(
          dataVencimento.getMonth() + (condicao.intervalo_dias / 30) * i,
        );
      ...
      // Formatar a data no formato YYYY-MM-DD diretamente sem usar toISOString
  ...
  // Substitua a função calcularParcelasCustomizadas por esta versão corrigida:
  const calcularParcelasCustomizadas = () => {
    ...
    for (let i = 0; i < customParcelas; i++) {
      ...
      if (i === 0) {
        ...
        // Para as demais parcelas, avança meses mantendo o dia do mês
        dataVencimento.setMonth(
          dataVencimento.getMonth() + (customIntervalo / 30) * i,
        );
        ...
        <div className="mb-6 p-4 bg-gray-50 rounded-lg">
          ...
          {usarDataPersonalizada && (
            <div className="mt-3">
              ...
              <p className="text-xs text-gray-500 mt-1">
                As demais parcelas serão geradas no mesmo dia dos meses
                seguintes
              </p>
            </div>
          )}
          ...
          {!usarDataPersonalizada && (
            <p className="text-sm text-gray-600">
              Primeira parcela será gerada para hoje, demais parcelas nos meses
              seguintes
            </p>
          )}
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {condicoes.map((condicao) => (
              ...
              >
                ...
                <div className="text-sm text-gray-600">
                  {condicao.numero_parcelas} parcela(s) -{" "}
                  {condicao.intervalo_dias} dias
                </div>
              </button>
            ))}
          </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCustomParcelas(parseInt(e.target.value) || 1)
                    }
                    className="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCustomIntervalo(parseInt(e.target.value) || 30)
                    }
                    className="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
                  <p className="text-xs text-gray-500 mt-1">
