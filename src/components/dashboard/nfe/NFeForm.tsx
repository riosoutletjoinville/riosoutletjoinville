...
export function NFeForm({ configFiscal, onSuccess }: NFeFormProps) {
  ...
  function possuiNotaFiscal(pedido: any) {
    ...
    return pedido.notas_fiscais.some(
      (nota: any) => nota.status !== "cancelada",
    );
  }
  ...
  // Verifica se existe nota com status autorizada ou processando
  const temNotaAtiva = pedido.notas_fiscais?.some(
    (nota: any) => nota.status === "autorizada" || nota.status === "processando"
  );
  
  if (temNotaAtiva) {
    notification.showWarning("Atenção", "Este pedido possui uma nota fiscal ativa (autorizada ou processando)");
    return;
  }
  ...
  const handleBuscaChange = (value: string) => {
    if (tipoBusca === "documento") {
      ...
      if (numeros.length <= 11) {
        ...
        if (numeros.length > 9)
          formatado = `${numeros.slice(0, 3)}.${numeros.slice(3, 6)}.${numeros.slice(6, 9)}-${numeros.slice(9, 11)}`;
        setBusca(formatado);
      } else {
        let formatado = numeros;
        if (numeros.length > 2)
        ...
        if (numeros.length > 12)
          formatado = `${numeros.slice(0, 2)}.${numeros.slice(2, 5)}.${numeros.slice(5, 8)}/${numeros.slice(8, 12)}-${numeros.slice(12, 14)}`;
  ...
  async function handleEmitir() {
    if (!pedidoSelecionado) {
      notification.showWarning(
        "Atenção",
        "Selecione um pedido para emitir a NF-e",
      );
      return;
    }
    ...
    try {
      ...
      if (statusNota === "autorizada") {
        ...
        notification.showInfo(
          ...
          "A nota foi enviada e está sendo processada pela SEFAZ.",
        );
      ...
      notification.showError(
        "Erro na Emissão",
        error instanceof Error ? error.message : "Erro na emissão",
      );
    } finally {
      setEmitindo(false);
    }
  ...
  function getTipoClienteIcon(cliente: any) {
    ...
    return cliente.tipo_cliente === "fisica" ? (
  ...
  const getPaginasMostrar = () => {
    ...
    if (totalPaginas <= maxPaginasMostrar) {
      ...
      if (paginaAtual <= 3) {
        ...
        for (
          let i = totalPaginas - maxPaginasMostrar + 1;
          i <= totalPaginas;
          i++
        ) {
          paginas.push(i);
            ...
            <div className="flex-1 flex gap-2 relative" ref={dropdownRef}>
              ...
              {showDropdown && (
                <div className="absolute top-full left-0 mt-1 w-[130px] bg-popover rounded-md border shadow-md z-50">
                  <button
                    onClick={() => handleTipoBuscaChange("nome")}
                    className="w-full px-3 py-2 text-sm flex items-center gap-2 hover:bg-accent rounded-t-md"
                  >
                    <User className="h-4 w-4" />
                    <span>Nome</span>
                  </button>
                  <button
                    onClick={() => handleTipoBuscaChange("documento")}
                    className="w-full px-3 py-2 text-sm flex items-center gap-2 hover:bg-accent rounded-b-md"
                  >
                    <CreditCard className="h-4 w-4" />
                    <span>CPF/CNPJ</span>
                  </button>
              ...
              <>
                <div className="divide-y">
                  {pedidos.map((pedido: any) => {
                    const temNotaAtiva =
                      pedido.notas_fiscais?.some(
                        (nota: any) => nota.status === "autorizada",
                      ) || false;

                    const statusNota = pedido.notas_fiscais?.[0]?.status;
                      ...
                      <Tooltip key={pedido.id}>
                        <TooltipTrigger asChild>
                          <div
                            ...
                            onClick={() => !temNotaAtiva && abrirModal(pedido)}
                          >
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm text-lime-800">
                                Este pedido já possui nota fiscal ativa
                ...
                {totalPaginas > 1 && (
                  <div className="flex items-center justify-between p-4 border-t bg-muted/20">
                    <div className="text-sm text-muted-foreground">
                      Mostrando {(paginaAtual - 1) * ITEMS_PER_PAGE + 1} a{" "}
                      {Math.min(paginaAtual * ITEMS_PER_PAGE, totalItens)} de{" "}
                      {totalItens} pedidos
                    </div>
                      ...
                      <div className="flex items-center gap-1">
                        {getPaginasMostrar().map((pagina) => (
                          <Button
                            key={pagina}
                            variant={
                              paginaAtual === pagina ? "default" : "outline"
                            }
                            size="sm"
                            className="w-8 h-8"
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
              <p className="text-xs text-blue-800">
                ⚠️ A emissão da NF-e será processada pela SEFAZ. Verifique os
          ...
          <DialogFooter className="gap-2">
            ...
            <Button
              ...
              disabled={emitindo || !pedidoSelecionado}
