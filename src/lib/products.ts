...
export class MercadoLivreProductsService {
  private async findAppropriateSizeGrid(
    ...
    const isInfantil =
      categoryName.toLowerCase().includes("infantil") ||
    ...
    try {
      ...
      console.error("Erro ao buscar grades via API, usando fallback:", error);
    }
  ...
  private findLocalSizeGrid(
    ...
    return (
      mlSizeGrids.find((grid) => {
        const genderMatch = grid.gender === gender;
        const sizeMatch =
          minSize >= (grid.minSize || 0) && maxSize <= (grid.maxSize || 50);
  ...
  private getPictureIdsForVariation(
    ...
    colorImages.forEach((colorImage) => {
      ...
      const originalIndex = images.findIndex((img) => img.id === colorImage.id);
    ...
    return variationPictureIds.length > 0 ? variationPictureIds : mlImageIds;
  }
  ...
  private logImageAssociation(
    ...
    variations.forEach((variation, index) => {
      ...
      console.log(`  - Imagens específicas encontradas: ${colorImages.length}`);
  ...
  private async getSizeChartDetails(
    ...
    try {
      console.log(`Buscando detalhes da tabela de medidas ID: ${chartId}`);

      // Para as grades que criamos, usar dados específicos em vez da API
      const predefinedChart = this.getPredefinedSizeChart(chartId);
      if (predefinedChart) {
        console.log("Usando grade predefinida:", predefinedChart.names.MLB);
      ...
      const fallbackChart =
        this.getPredefinedSizeChart(chartId) ||
    ...
    if (productData.variations) {
      productData.variations.forEach((variation, index) => {
        if (variation.price <= 0) {
          errors.push(`Variação ${index + 1}: Preço deve ser maior que zero`);
        }
        if (variation.available_quantity < 0) {
          errors.push(
            `Variação ${index + 1}: Quantidade não pode ser negativa`,
          );
        }
        if (
          ...
          errors.push(
            `Variação ${index + 1}: Atributos de combinação são obrigatórios`,
          );
        }
      });
    }
    const hasModel = productData.attributes.some((attr) => attr.id === "MODEL");
    if (!hasModel) {
  ...
  private normalizeDescriptionForGender(
    ...
    if (!needsShoePrefix) {
      ...
      if (isInfantil) {
        if (gender === "Masculino" || gender === "Menino") {
          finalDescription = `Tênis Infantil Menino ${finalDescription}`;
        } else if (gender === "Feminino" || gender === "Menina") {
          finalDescription = `Tênis Infantil Menina ${finalDescription}`;
        } else {
  ...
  private async prepareProductData(
    ...
    const modelName = product.modelo_prod || product.titulo.substring(0, 60);
    ...
    const sizeChart = await this.getSizeChartDetails(sizeGridId, accessToken);

    this.logSizeGridSelection(variations, categoryName, genderName, sizeGridId);
    ...
    try {
      const shippingSettings = await this.getUserShippingSettings(accessToken);
    ...
    if (variations && variations.length > 0) {
      const variationsData: MLVariation[] = variations.map((variation) => {
        ...
        const variationPictureIds = mlImageIds
          ? this.getPictureIdsForVariation(variation.cor_id, images, mlImageIds)
          : [];
  ...
  private getPredefinedSizeChart(chartId: string): MLSizeChart | null {
    const charts: Record<string, MLSizeChart> = {
      ...
      "3979561": {
        // ✅ NOVA: Tênis Feminino Adulto 32-42
        id: "3979561",
      ...
      "3733398": {
        // Tênis Infantil Masculino 16-33
        id: "3733398",
      ...
      "3839830": {
        // ✅ NOVA: Tênis Infantil Masculino 16-35
        id: "3839830",
  ...
  private async debugProductData(productId: string, accessToken: string) {
    try {
      const productData = await this.prepareProductData(productId, accessToken);
...
private async validateToken(accessToken: string): Promise<boolean> {
  try {
    ...
    console.log(`✅ Token válido: ${isValid}, Status: ${response.status}`);
    
    if (!isValid && response.status === 401) {
      console.log("⚠️ Token expirado ou inválido, será necessário renovar");
    }
  ...
  private async uploadImagesToML(
    ...
    for (const image of images.slice(0, 4)) {
      try {
        ...
        const uploadResponse = await fetch("/api/mercadolibre/upload-image", {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
            "Content-Type": "application/json",
  ...
  async getUserProducts(supabaseClient?: SupabaseClient): Promise<MLProduct[]> {
    try {
      console.log("🟡 [getUserProducts] Iniciando busca de produtos...");

      const client = supabaseClient || supabase;

      // ✅ Buscar token usando o client correto
      ...
      console.log("🔍 [getUserProducts] Token encontrado?", !!accessToken);
      ...
      console.log("🌐 [getUserProducts] Fazendo requisição para:", apiUrl);
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          console.error("❌ [getUserProducts] Erro 401 - Autenticação falhou");
          ...
          try {
            ...
            console.error("❌ [getUserProducts] Detalhes do erro:", errorData);
          } catch (e) {
            console.error(
      ...
      console.log("📊 [getUserProducts] Produtos recebidos:", {
        ...
        primeirosIds: productsData.slice(0, 3).map((p: MLProduct) => p.id),
      });
  ...
  async publishProduct(
    ...
    try {
      ...
      const { product, variations, images } =
        await this.getProductDetails(productId);
  ...
  async updateProduct(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || `Erro ao atualizar produto: ${response.status}`,
        );
      }
  ...
  async updateStock(
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || `Erro ao atualizar estoque: ${response.status}`,
        );
      }
  ...
  async pauseProduct(productId: string): Promise<{ status: string }> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || `Erro ao pausar produto: ${response.status}`,
        );
      }
  ...
  async activateProduct(productId: string): Promise<{ status: string }> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || `Erro ao reativar produto: ${response.status}`,
        );
      }
    ...
    try {
      ...
      if (!statusResponse.ok) {
        if (statusResponse.status === 404) {
          console.log("ℹ️ [deleteProduct] Produto já não existe no ML");
      ...
      if (productData.status === "closed") {
        ...
        return {
          success: false,
          message: `❌ Não é possível excluir produto com status "${productData.status}". Apenas produtos "active", "paused" ou "closed" podem ser excluídos.`,
        };
      }
      ...
      if (needsClosing) {
        ...
        console.log("⏳ [deleteProduct] Aguardando 2 segundos após fechar...");
      ...
      console.log("✅ [deleteProduct] Produto excluído com sucesso no ML");
      await this.cleanupLocalData(productId);
      return {
        success: true,
  ...
  private async cleanupLocalData(productId: string): Promise<void> {
    if (productId && !productId.startsWith("temp_")) {
      ...
      if (error) {
        console.error("❌ [cleanupLocalData] Erro ao limpar dados:", error);
      } else {
  ...
  async testDirectDelete(
    ...
    try {
      ...
      console.log("📊 [testDirectDelete] Status ANTES:", checkBefore.status);
      ...
      if (productData.status !== "closed") {
        console.log("🔄 [testDirectDelete] PASSO 1 - Fechando anúncio...");
        const closeResponse = await fetch(
          `https://api.mercadolibre.com/items/${mlItemId}`,
          {
            method: "PUT",
        ...
        if (!closeResponse.ok) {
          ...
          return {
            success: false,
            message: `❌ Erro ao fechar anúncio: ${closeResponse.status} - ${errorBody}`,
          };
        }
      ...
      // ✅ PASSO 2: MARCAR COMO DELETADO
      console.log("🗑️ [testDirectDelete] PASSO 2 - Marcando como DELETADO...");
      const deleteResponse = await fetch(
        `https://api.mercadolibre.com/items/${mlItemId}`,
        {
          method: "PUT",
      ...
      console.log("📄 [testDirectDelete] Corpo da resposta:", responseBody);
      ...
      if (!deleteResponse.ok) {
        ...
        if (deleteResponse.status === 409) {
          ...
          if (!retryResponse.ok) {
            ...
            return {
              success: false,
              message: `❌ Erro na segunda tentativa: ${retryResponse.status} - ${retryBody}`,
            };
          }
        } else {
          return {
            success: false,
            message: `❌ Erro ao deletar: ${deleteResponse.status} - ${responseBody}`,
          };
        }
      }
      ...
      console.log("📊 [testDirectDelete] Status DEPOIS:", checkAfter.status);
      ...
      if (checkAfter.status === 404) {
        ...
        if (checkAfter.ok) {
          ...
          return {
            success: false,
            message: `❌ FALHA NA EXCLUSÃO! Produto ainda existe. Status atual: ${afterData.status}`,
          };
        } else {
          return {
            success: false,
            message: `❌ FALHA NA EXCLUSÃO! Status da verificação: ${checkAfter.status}`,
          };
        }
      }
    } catch (error) {
  ...
  async closeProduct(productId: string): Promise<{ status: string }> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || `Erro ao fechar produto: ${response.status}`,
        );
      }
